# Vault Index — [YOUR_NAME]

> Navigation hub. All major notes linked here.

---

## Core Files

- [[CLAUDE.md]] — Behavior blueprint for Claude
- [[Tasks.md]] — All active work
- [[Active Batch.md]] — Current batch of tasks in progress

---

## Notes

- `Notes/Business/` — Project work states, client notes, automations
- `Notes/People/` — Everyone Claude interacts with on your behalf
- `Notes/Claude Memory/` — Auto-generated memory files
- `Notes/Playbooks/` — Step-by-step workflows for repeatable tasks
- `Notes/Inner Work/` — Personal goals, frameworks, mindset work

---

## Playbooks

- [[Notes/Playbooks/Batch Planning Playbook]] — Plan a full week in one session
- [[Notes/Playbooks/Testing and Delivery Protocol]] — QA before shipping anything

---

## Daily Notes

Recent notes live in `Daily Notes/`. Claude creates today's note automatically.

---

*Updated automatically. If a major note isn't linked here, add it.*
