...
**No, it cannot be run on an emulator here**, and that was checked three ways rather than assumed: there is no /dev/kvm and no virtualisation CPU flags, so an emulator would fall back to software translation; the SDK, emulator and system images all come from a host the network policy blocks; and there is nothing to install on it anyway because the app cannot be built.

One serious attempt was made at an alternative. Robolectric runs real Android framework code on the JVM with no emulator at all, and its framework jars are on Maven Central. AlarmScheduler was split into its own module and decoupled from the app's UI classes specifically so it could be tested in isolation, and thirteen tests were written for it — that something reaches AlarmManager, that it uses the Doze-exempt API rather than a throttleable one, that rescheduling replaces instead of waking someone twice, and that without exact-alarm permission it throws rather than pretending the alarm is armed. Robolectric itself depends on a Google-hosted library that is not mirrored anywhere reachable, so it does not run. The refactor and the tests were kept rather than reverted: they run as written on any machine with normal network access.
