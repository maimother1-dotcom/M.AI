...
The governing fact, and the design principle worth carrying to every future product: **never promise a fixed reward against variable revenue.** Pay a SHARE of income that has already arrived, verified server-side, and the loss case stops existing. AlarmX pays 50–60% of realised ad revenue, rising with the user's streak. ₹95/month, ₹0.50 a math and ₹2 a set all failed the same way — a number picked first and defended afterwards. Positioned for exam students, Hinglish by default. Any future feature that pays users has to be checked against `economics/AlarmX-unit-economics.xlsx` before it ships.
...
Three design flaws were caught and fixed before build: a 10-second alarm that auto-dismissed (now the alarm rings until dismissed and 10s is only the reward window), a withdrawal bottleneck that was a Play Store removal risk (now pay out in full, always — easy to promise honestly once payouts are a share of money already collected), and a fake "technical difficulty" message for flagged accounts (now an honest under-review state with appeals).

Retention insight worth reusing: **a capped product cannot buy attention with money, so borrow a habit that already exists.** v0.4 added a Bengali panjika / Tamil daily calendar and a 4×2 home-screen widget. The calendar is the reason to look, AlarmX state is what is seen while looking — no notification needed, no rupees spent, and the revenue share is untouched. It also merged two audiences that looked incompatible: one widget carries both the tithi and the exam countdown.
...
Related technique: **put the guarantee in the build, not the document.** `build_model.py` refuses to write the workbook if any scenario at any share tier at a quarter of the assumed eCPM would lose money — 36 combinations, and the gate was tested by deliberately breaking it. A spreadsheet nobody re-reads is not a control.

Second: **a monthly cap creates the failure it is meant to prevent.** A user who hits it earns nothing for the rest of the cycle, which is the opposite of retention. A per-day ceiling bounds fraud without that. Bijoy's three requirements were "open it daily, feel paid at day's end, never lose money" — the cap was quietly working against the first two.
