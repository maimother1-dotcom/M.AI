...
source: correction
---
Never report the per-deployment hash URL (e.g. `my-app-abc123xyz-team.vercel.app`) when telling [USER] something is live. Always use the stable production alias: `<project>.vercel.app` or `<project>-<team>.vercel.app`. Vercel automatically aliases the latest prod deployment to these — they don't change across deploys.

**Why:** [USER] doesn't want a new URL after every deploy. Changing URLs break Supabase `site_url`, OAuth redirects, client bookmarks, and anything pointing at the app.
